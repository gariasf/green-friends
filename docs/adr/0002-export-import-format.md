...
- **Import is always a merge**: per-row LWW upsert by UUID — newer `updated_at` wins, tombstones beat older edits. At most one live photo per plant holds after the merge too: where each device gave a plant a new photo, the newest stays live, as if it had been taken last, and the others are Deleted, their rows kept as tombstones. Importing into an empty install *is* the restore path; there is no import-mode picker. Hard reset to a backup = "Erase all data" in settings, then import.
...
- **Version policy**: an older `schema_version` is migrated forward on import (forward-only, same machinery as DB migrations: the rows are loaded into an empty database in memory at their version and brought forward by the very migrations the app runs on itself; that scratch copy is discarded after, and the data stays in ADR-0001's single database); a newer one than the app knows is refused with "update the app".
...
- Import checks rows by the mutations' rules of the importing app, so tightening a rule later needs a migration that brings older rows in line, or older Exports stop importing.
- LWW keeps each row's newest version, not each field's (the later per-field sync does), so across two devices a rule spanning rows can come apart: a plant edited on one device after the other Deleted it comes back without the Care Log and photo that deletion took; a Care Event or photo added to a plant the other device had Deleted stays beside it, never shown; a repot sets the Current Pot only where its plant row is also the newer. Only the one-live-photo rule is restored after a merge, since a second live photo would list the plant twice.
