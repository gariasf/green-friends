...
Green Friends is an Expo (CNG) app: `app/` holds expo-router routes, `src/core` the plain-TypeScript domain, `src/db` the Drizzle schema, migrations and the expo-sqlite handle, `src/ui` shared components, `scripts/` build-time tooling, TypeScript run directly by Node 24.3 or later (`engines` in `package.json`). See ADR-0001.
