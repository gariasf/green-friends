...
- Styling comes from `src/ui/theme.ts`, the only file holding colour values: iOS semantic colours and brand green, which follow light and dark mode natively, plus the type scale, spacing steps and grouped-list styles. React Native draws text black in either appearance, so every `Text` takes a `text.*` style or a colour from `colors`. Icons are SF Symbols (`expo-symbols`); a Care Event type's symbol and hue live in `CARE_COPY` (`src/ui/CareEvent.tsx`). Every `Pressable` shows a pressed state and is a 44 pt target, with `hitSlop` where it's drawn smaller. Screens animate changes with Reanimated's layout animations: React Native's `LayoutAnimation` aborted the app once (SIGABRT in `LayoutAnimationDelegateProxy`, a sheet closing during Today's refresh).
