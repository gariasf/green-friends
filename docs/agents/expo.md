...
- Every write goes through a `src/core` mutation function; components never call `db.insert/update/delete`. Screens that must react to writes subscribe with Drizzle's `useLiveQuery` to a query builder the core exports (`plantListQuery`); it re-runs on changes to the query's primary table only. A screen over state derived from several tables (Today, over `evaluateCare`) instead re-evaluates after writes through expo-sqlite's `addDatabaseChangeListener`, debounced because it fires once per changed row, and when the app returns to the foreground.
