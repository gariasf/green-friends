...
- Styling comes from `src/ui/theme.ts`, the only file holding colour values: iOS semantic colours and brand green, which follow light and dark mode natively; the navigation theme, in hex and picked by `useColorScheme()`; the type scale, spacing steps, pressed styles and grouped-list styles. React Native draws text black in either appearance, so every `Text` takes a `text.*` style or a colour from `colors`. Icons are SF Symbols (`expo-symbols`); a Care Event type's symbol and hue live in `CARE_COPY`, and `CareSymbol` draws them (`src/ui/CareEvent.tsx`). Every `Pressable` shows a `pressedStyle` and is a 44 pt target, with `hitSlop` where it's drawn smaller. `hitSlop` reaches only as far as the nearest ancestor that clips (`overflow: 'hidden'`, a ScrollView), so that ancestor needs the padding; a header button needs the 44 pt as its own size, since UIKit routes touches to header items. Screens animate changes with Reanimated's layout animations: React Native's `LayoutAnimation` aborted the app once (SIGABRT in `LayoutAnimationDelegateProxy`, a sheet closing during Today's refresh).
